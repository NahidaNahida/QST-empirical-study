"Templates for generating LaTeX tables"
...
def count_latex_columns(col_def: str) -> int:
    ...
    # Remove modifiers like >{...} and <{...} since they are not columns
...
def vertical_tables(
    ...
    if_cmidrule: bool = False,   # Whether \cmidrule follows multiple rows
...
def vertical_grouped_table(
    data: dict,
    save_path: str,
    tab_space: str = "c|c",  # Number of columns per row, usually based on sub-dictionary length
    addition_line: str | None = None
):
    """
    ...
    """
        ...
        # Add a separating rule if not the last key
...
def two_dimensional_table(
    ...
    # Rows and columns (sorted by dictionary keys; can be changed to keep original order)
    ...
    # Alignment: first column is l, remaining columns are c
    tab_space = "c|" + "c" * len(cols)

    # Table header (first row)
    headers_str = " & ".join([""] + cols)  # First cell is empty for row names

    # cmidrule
    # If there are C columns, the rule is: \cmidrule(r){2-<C+1>}
    ...
    # Optional extra line
    addition_line = ""  # Default is empty
