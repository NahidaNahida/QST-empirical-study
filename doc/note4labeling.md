...
+ `Scalability-#OfGates`: The number of quantum gates of the quantum circuit under test (setting instead of metrics).
  + **[Total]**: The total account of quantum gates if the paper offers this info.
  + **[Specific]**: The number of specific quantum gates based on the info provided by the paper.
...
+ `TestOracle-OutputExpectation`:
  ...
  + `[Metamorphic relation]`: A program output can be regarded correct if the metamorphic relation is satisfied. However, satisfying an MR does not guarantee that the program output (or the program itself) is absolutely correct.
